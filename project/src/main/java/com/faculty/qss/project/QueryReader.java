...
import com.faculty.qss.project.comands.Database;
import com.faculty.qss.project.comands.TableWithoutLibrary;
...
public class QueryReader {
	...
	public void readQuery() throws IOException {
		...
		while (true) {
			...
			if (splited.length == 4 && splited[0].equals("create") && splited[1].equals("new")
					&& splited[2].equals("database")) {
			...
			if (splited.length == 4 && splited[0].equals("show") && splited[1].equals("tables")
					&& splited[2].equals("from")) {
				database.showTablesFromDatabase(splited[3]);
			}
			if (splited.length == 4 && splited[0].equals("change") && splited[1].equals("database")
					&& splited[2].equals("to")) {
			...
			if (database.getDatabaseName() != null) {
				...
				if (splited.length > 2 && splited[0].equals("insert") && splited[1].equals("into")) {
					...
					ArrayList<String> values = (ArrayList<String>) Stream.of(queryDivider[1].split(","))
							.map(elem -> elem.replace(" ", "").replace(")", "")).collect(Collectors.toList());
					table.insertRecord(database.getDatabaseName(), tableName, values);
				}
				if (splited.length > 4 && splited[0].equals("delete") && splited[1].equals("from")
						&& splited[3].equals("where")) {
...
//                WHERE condition;
				if (splited.length > 5 && splited[0].equals("update") && command.contains("set")
